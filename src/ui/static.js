...
function downloadCertificate(name,inst,lv){const c=document.createElement('canvas');c.width=1200;c.height=800;const x=c.getContext('2d');x.fillStyle='#0c130f';x.fillRect(0,0,1200,800);x.strokeStyle='#e9b84e';x.lineWidth=10;x.strokeRect(35,35,1130,730);x.textAlign='center';x.fillStyle='#e9b84e';x.font='bold 110px sans-serif';x.fillText('ቅኝት',600,180);x.fillStyle='#f3ead9';x.font='32px sans-serif';x.fillText('CERTIFICATE OF ACHIEVEMENT',600,270);x.font='bold 58px sans-serif';x.fillText(name,600,390);x.fillStyle='#c7cabf';x.font='32px sans-serif';x.fillText(`${inst.en} · ${lv.en}`,600,475);x.font='24px sans-serif';x.fillText(new Date().toLocaleDateString(),600,550);x.fillStyle='#e9b84e';x.fillText('Axiom Digital',600,670);const a=document.createElement('a');a.download='kinit-certificate.png';a.href=c.toDataURL('image/png');a.click();}

export function renderOnboarding(app){const v=el('div','view onboarding');let step=0,inst='keyboard',level='beginner';const draw=()=>{const pages=[`<div class="onboard-icon">♪</div><h1>እንኳን ደህና መጡ</h1><p>Learn by hearing, seeing and playing. Every sound works offline.</p>`,`<h1>መሣሪያዎን ይምረጡ</h1><div class="choice-grid">${Object.entries(INSTRUMENTS).map(([k,i])=>`<button data-inst="${k}" class="choice ${k===inst?'on':''}">${i.ic}<span>${i.am}</span></button>`).join('')}</div>`,`<h1>ደረጃዎን ይምረጡ</h1><div class="choice-grid">${Object.entries(LEVELS).map(([k,l])=>`<button data-level="${k}" class="choice ${k===level?'on':''}">${l.am}<span>${l.en}</span></button>`).join('')}</div>`];v.innerHTML=`<div class="onboard-card"><div class="dots">${[0,1,2].map(i=>`<i class="${i===step?'on':''}"></i>`).join('')}</div>${pages[step]}<button class="btn" data-next>${step<2?'ቀጥል · Continue':'ጀምር · Start'}</button></div>`;v.querySelectorAll('[data-inst]').forEach(b=>b.onclick=()=>{inst=b.dataset.inst;draw()});v.querySelectorAll('[data-level]').forEach(b=>b.onclick=()=>{level=b.dataset.level;draw()});v.querySelector('[data-next]').onclick=()=>{if(step<2){step++;draw()}else{app.data.onboarded=true;saveStore(app.data);app.openLevel(inst,level)}}};draw();return v;}
